...
-- This resolves the NULL values issue in City Health display

-- Drop existing view completely to allow data type changes
...
SELECT
  ...
  -- ✅ REAL EVENT COUNTS (no more dummy zeros!)
  ...
  -- ⭐ FREE EVENTS COUNT (the critical metric!)
  COUNT(DISTINCT e.id) FILTER (
    WHERE e.status = 'active' 
    AND e.price = 0
    AND e.start_time >= NOW()  -- Only upcoming free events
  ...
  -- Average confidence from event_confidence table
