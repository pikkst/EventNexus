...
DROP POLICY IF EXISTS "Admins can view all code redemptions" ON public.code_redemptions;
DROP POLICY IF EXISTS "Users can view own code redemptions" ON public.code_redemptions;
DROP POLICY IF EXISTS "Users can redeem codes" ON public.code_redemptions;

-- Drop existing view
DROP VIEW IF EXISTS public.promo_code_stats;

-- Drop existing functions
DROP FUNCTION IF EXISTS public.redeem_promo_code(UUID, VARCHAR);
DROP FUNCTION IF EXISTS public.admin_grant_credits(UUID, INTEGER, TEXT, UUID);

-- Create or update promo_codes table
...
-- Create or update credit_transactions table
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='credit_transactions' AND column_name='performed_by') THEN
    ALTER TABLE public.credit_transactions ADD COLUMN performed_by UUID REFERENCES auth.users(id) ON DELETE SET NULL;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='credit_transactions' AND column_name='metadata') THEN
    ALTER TABLE public.credit_transactions ADD COLUMN metadata JSONB DEFAULT '{}'::jsonb;
  END IF;
END $$;

-- Create or update code_redemptions table
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'code_redemptions_promo_code_id_user_id_key'
  ) THEN
    ALTER TABLE public.code_redemptions ADD CONSTRAINT code_redemptions_promo_code_id_user_id_key UNIQUE(promo_code_id, user_id);
  END IF;
END $$;
...
-- Recreate RLS Policies for promo_codes
...
-- Recreate RLS Policies for credit_transactions
...
-- Recreate RLS Policies for code_redemptions
...
-- Recreate function to validate and redeem a code
...
-- Recreate function for admins to grant credits directly
...
-- Recreate view for code statistics
