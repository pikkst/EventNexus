-- Update revenue functions to exclude archived events from revenue calculations
-- This allows organizers to archive completed events without affecting their stats

-- Drop and recreate get_organizer_revenue function with archived filter
...
BEGIN
  ...
  WHERE e.organizer_id = org_id
    AND e.archived_at IS NULL  -- Exclude archived events
  GROUP BY e.id, e.name, e.date, u.subscription_tier, p.status, p.processed_at
  ORDER BY e.date DESC;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Drop and recreate get_organizer_revenue_summary function with archived filter
...
COMMENT ON FUNCTION get_organizer_revenue(UUID) IS 'Get revenue breakdown by event for an organizer, excluding archived events';
COMMENT ON FUNCTION get_organizer_revenue_summary(UUID) IS 'Get revenue summary totals for an organizer, excluding archived events';
