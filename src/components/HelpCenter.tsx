...
const HelpCenter: React.FC = () => {
  ...
  const categories = [
    { icon: <Users />, name: t.help.categories.attendee, description: t.help.categories.attendeeDesc },
    { icon: <Zap />, name: t.help.categories.organizer, description: t.help.categories.organizerDesc },
    { icon: <CreditCard />, name: t.help.categories.billing, description: t.help.categories.billingDesc },
    { icon: <ShieldCheck />, name: t.help.categories.safety, description: t.help.categories.safetyDesc }
  ];
    ...
    <div className="max-w-5xl mx-auto px-4 py-20 space-y-20 animate-in fade-in duration-700">
      ...
      <div className="text-center space-y-8">
        <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-white">
          {t.help.title.includes('?') ? t.help.title.replace('?', '') : t.help.title.split(' ').slice(0, -1).join(' ')} 
          <span className="text-indigo-500">
            {t.help.title.includes('?') ? '?' : ' ' + t.help.title.split(' ').slice(-1) + '?'}
          </span>
        </h1>
      ...
      <div className="space-y-8">
        <h2 className="text-3xl font-black tracking-tighter text-white text-center">{t.help.faqTitle}</h2>
      ...
      <div className="bg-indigo-600 rounded-[48px] p-12 text-center space-y-6 relative overflow-hidden shadow-2xl shadow-indigo-600/20">
        ...
        <h2 className="text-3xl font-black text-white">{t.help.contactTitle}</h2>
        <p className="text-indigo-100 max-w-lg mx-auto font-medium">{t.help.contactSubtitle}</p>
