...
/**
 * Contact inquiry from agency public page
 */
...
/**
 * Send contact form inquiry to organizer
 */
export async function sendContactInquiry(inquiry: ContactInquiry): Promise<boolean> {
  try {
    if (!RESEND_API_KEY) {
  ...
  <div style="background: white; padding: 40px 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 16px 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
    ...
    <div style="background: #f3f4f6; padding: 20px; border-radius: 12px; margin-bottom: 30px; border-left: 4px solid ${isPartnership ? '#8b5cf6' : '#6366f1'};">
      <div style="display: flex; align-items: center; margin-bottom: 15px;">
        <div style="background: ${isPartnership ? '#8b5cf6' : '#6366f1'}; color: white; width: 48px; height: 48px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 20px; font-weight: bold; margin-right: 15px;">
    ...
    const data = await response.json();
    console.log('✅ Contact inquiry email sent:', data.id);
    ...
    try {
      ...
      console.warn('Failed to store inquiry in database:', dbError);
      // Still return true since email was sent
    }
