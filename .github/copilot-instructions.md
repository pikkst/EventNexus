...
- **React Hooks Order (CRITICAL):** ALL hooks (useState, useRef, useEffect, useMemo, useCallback) MUST be declared at the top of the component in consistent order. NEVER declare state after effects or memos. This causes "Cannot access 'X' before initialization" errors in production builds due to Temporal Dead Zone violations. Pattern:
  ```typescript
  const MyComponent = () => {
    ...
    // 2. Then: all useRef declarations
    const ref1 = useRef();
    
    // 3. Then: all useEffect declarations
    useEffect(() => {}, []);
    
    // 4. Then: all useMemo/useCallback declarations
    const memo = useMemo(() => {}, []);
    
    // 5. Finally: regular functions and JSX
  }
  ```
